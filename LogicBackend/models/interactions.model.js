...
async function getInteractions(skip, limit) {
    try {
        return await Interaction.find().skip(skip).limit(limit).populate('user_id', 'name email').populate('product_id', 'name price');
    } catch (error) {
...
async function getInteraction(interaction_id) {
    try {
        const interaction = await Interaction.findById(interaction_id).populate('user_id', 'name email').populate('product_id', 'name price');
        if (!interaction) {
...
async function getProductInteractions(productId) {
    try {
        return await Interaction.find({ product_id: productId, interaction_type: INTERACTION_TYPES.RATING }).populate('user_id', 'name').sort({ createdAt: -1 });
    } catch (error) {
...
async function getUserInteractions(userId) {
    try {
        return await Interaction.find({ user_id: userId }).populate('product_id', 'name price').sort({ createdAt: -1 });
    } catch (error) {
