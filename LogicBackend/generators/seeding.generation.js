...
// Done
async function createUsers(count = 2) {
    let data1 = []
    ...
    return new Promise((resolve, reject) => {
        fs.createReadStream(filePath)
            ...
            .on('data', (row) => {
                ...
                const discountAmount = faker.number.float({ min: 1, max: 20, precision: 0.01 });
                const discounted_price = hasDiscount ? Math.max(price - discountAmount, 0) : 0;
                const product = {
